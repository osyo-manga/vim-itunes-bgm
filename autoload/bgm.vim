...
call vital#of("bgm").unload()
let s:V = vital#of("bgm")
function! bgm#get_vital()
	return s:V
endfunction


let s:Reunions = s:V.import("Reunions")
function! bgm#get_reunions()
	return s:Reunions
endfunction
...
let s:play_list = bgm#playlist#make()
...
function! bgm#start_by_request(request)
	call s:start(a:request)
endfunction


function! bgm#start_by_term(term)
	return bgm#start_by_request({ "term" : a:term, "country" : "JP", "entity" : "musicTrack" })
endfunction
